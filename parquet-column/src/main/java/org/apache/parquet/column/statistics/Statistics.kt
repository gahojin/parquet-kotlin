...
abstract class Statistics<T : Comparable<T>> internal constructor(
     ...
     * the null count
     *
     * @return null count or `-1` if the null count is not set
     */
    @set:Deprecated("will be removed in 2.0.0. Use {@link #getBuilderForReading(PrimitiveType)} instead.")
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return type == stats.type &&
                stats.maxBytes.contentEquals(this.maxBytes) &&
                stats.minBytes.contentEquals(this.minBytes) &&
                stats.numNulls == this.numNulls
    }
    ...
    override fun hashCode(): Int {
        return (31 * type.hashCode() + 31 * this.maxBytes.contentHashCode() + 17 * this.minBytes.contentHashCode() + this.numNulls.hashCode())
    }
    ...
    /**
     * Returns the min value in the statistics. The java natural order of the returned type defined by [ ][Comparable.compareTo] might not be the proper one. For example, UINT_32 requires unsigned comparison instead of the
     * natural signed one. Use [.compareMinToValue] or the comparator returned by [.comparator] to
    ...
    /**
     * Returns the max value in the statistics. The java natural order of the returned type defined by [ ][Comparable.compareTo] might not be the proper one. For example, UINT_32 requires unsigned comparison instead of the
     * natural signed one. Use [.compareMaxToValue] or the comparator returned by [.comparator] to
     * always get the proper ordering.
     *
        ...
        if (this.hasNonNullValue()) {
            if (this.isNumNullsSet) {
                return String.format(
                    "min: %s, max: %s, num_nulls: %d", minAsString(), maxAsString(), this.numNulls
                )
            } else {
                return String.format("min: %s, max: %s, num_nulls not defined", minAsString(), maxAsString())
            }
        } else if (!this.isEmpty) return String.format("num_nulls: %d, min/max not defined", this.numNulls)
    ...
    open val isEmpty: Boolean
         ...
         */
        get() = !hasNonNullValue && !this.isNumNullsSet
    ...
    companion object {
        ...
        fun getStatsBasedOnType(type: PrimitiveTypeName): Statistics<*> {
            when (type) {
                PrimitiveTypeName.INT32 -> return IntStatistics()
                PrimitiveTypeName.INT64 -> return LongStatistics()
                PrimitiveTypeName.FLOAT -> return FloatStatistics()
                PrimitiveTypeName.DOUBLE -> return DoubleStatistics()
                PrimitiveTypeName.BOOLEAN -> return BooleanStatistics()
                PrimitiveTypeName.BINARY -> return BinaryStatistics()
                PrimitiveTypeName.INT96 -> return BinaryStatistics()
                PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY -> return BinaryStatistics()
                else -> throw UnknownColumnTypeException(type)
            }
        }
        ...
        fun createStats(type: Type): Statistics<*> {
            ...
            when (primitive.getPrimitiveTypeName()) {
                PrimitiveTypeName.INT32 -> return IntStatistics(primitive)
                PrimitiveTypeName.INT64 -> return LongStatistics(primitive)
                PrimitiveTypeName.FLOAT -> return FloatStatistics(primitive)
                PrimitiveTypeName.DOUBLE -> return DoubleStatistics(primitive)
                PrimitiveTypeName.BOOLEAN -> return BooleanStatistics(primitive)
                PrimitiveTypeName.BINARY, PrimitiveTypeName.INT96, PrimitiveTypeName.FIXED_LEN_BYTE_ARRAY -> return BinaryStatistics(
                    primitive
                )

                else -> throw UnknownColumnTypeException(primitive.getPrimitiveTypeName())
            }
        }
        ...
        fun noopStats(type: Type): Statistics<*> {
            return NoopStatistics(type.asPrimitiveType())
        }
