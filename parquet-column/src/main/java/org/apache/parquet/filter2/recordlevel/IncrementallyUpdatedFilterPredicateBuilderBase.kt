 ...
 */
abstract class IncrementallyUpdatedFilterPredicateBuilderBase : FilterPredicate.Visitor<IncrementallyUpdatedFilterPredicate> {
    private var built = false

    private val comparatorsByColumn = hashMapOf<ColumnPath, PrimitiveComparator<*>>()

    val valueInspectorsByColumn = hashMapOf<ColumnPath, MutableList<ValueInspector>>()
    ...
    fun build(pred: FilterPredicate): IncrementallyUpdatedFilterPredicate {
        require(!built) { "This builder has already been used" }
        val incremental = pred.accept<IncrementallyUpdatedFilterPredicate>(this)
        built = true
        return incremental
    }

    protected fun addValueInspector(columnPath: ColumnPath, valueInspector: ValueInspector) {
        var valueInspectors = valueInspectorsByColumn.getOrPut(columnPath) { ArrayList() }
