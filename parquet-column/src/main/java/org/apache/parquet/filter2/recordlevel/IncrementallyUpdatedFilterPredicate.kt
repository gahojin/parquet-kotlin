...
interface IncrementallyUpdatedFilterPredicate {
    ...
    open class ValueInspector internal constructor() : IncrementallyUpdatedFilterPredicate {
        ...
        protected fun setResult(result: Boolean) {
            check(!isKnown) { "setResult() called on a ValueInspector whose result is already known! Did you forget to call reset()?" }
            this.result = result
            isKnown = true
        }
     ...
     */
    abstract class DelegatingValueInspector internal constructor(
        vararg delegates: ValueInspector,
    ) : ValueInspector() {
        val delegates: Iterable<ValueInspector> = delegates.toList()
