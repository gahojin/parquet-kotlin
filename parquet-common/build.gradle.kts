...
val artifactId = "parquet-common"

group = Maven.GROUP_ID
version = Maven.VERSION

dependencies {
    implementation(project(":parquet-format-structures"))
...
tasks.register<GeneratorTask>("versionGenerator") {
    ...
    generatorClass = "org.apache.parquet.version.Generator"
    outputDir = project.layout.buildDirectory.dir("generated/sources/generator")
}

tasks.withType<DokkaGenerateTask> { dependsOn("versionGenerator") }
tasks.withType<Jar> { dependsOn("versionGenerator") }
tasks.findByName("compileKotlin")?.dependsOn("versionGenerator")
